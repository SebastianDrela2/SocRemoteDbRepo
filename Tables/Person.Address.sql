...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [Person].[AddressModified]
ON [Person].[Address] AFTER UPDATE NOT FOR REPLICATION
AS
UPDATE d SET ModifiedDate = getdate()
FROM [Person].[Address] d inner join inserted i on d.Modifieddate = i.Modifieddate AND d.[AddressLine1] = i.[AddressLine1] AND d.[City] = i.[City];
GO
